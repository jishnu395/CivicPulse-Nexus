...
| Milestone | Requirement | Backend Service | API Endpoint | Database | Kafka Topic/Event | Frontend Page/Component | Role | Validation | Testing Evidence | Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :---: |
| **M3** | Welfare Scheme Catalog & CRUD | `welfare-service` | `POST /api/welfare/schemes`, `GET /api/welfare/schemes`, `PUT /api/welfare/schemes/{id}` | `welfare_db.welfare_schemes` | - | `WelfareSchemesPage.tsx` | ADMIN / COMMISSIONER | Scheme budget, eligibility criteria | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Citizen Welfare Scheme Application | `welfare-service` | `POST /api/welfare/applications/apply`, `GET /api/welfare/applications/my/{citizenId}` | `welfare_db.welfare_applications` | - | `ApplyWelfarePage.tsx`, `MyWelfareApplicationsPage.tsx` | CITIZEN | Annual income, age, criteria check | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Eligibility Verification & Application Approval | `welfare-service` | `GET /api/welfare/applications/pending`, `PUT /api/welfare/applications/{id}/approve`, `PUT /api/welfare/applications/{id}/reject` | `welfare_db.welfare_applications` | - | `WelfareManagementPage.tsx` | OFFICER / ADMIN | Automatic & manual eligibility checks | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Beneficiary Enrollment & Tracking | `welfare-service` | `POST /api/welfare/beneficiaries/register/{applicationId}`, `GET /api/welfare/beneficiaries` | `welfare_db.beneficiaries` | - | `WelfareManagementPage.tsx` | OFFICER / ADMIN | Duplicate beneficiary prevention | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Budget Planning & Annual/Quarterly Creation | `budget-service` | `POST /api/budget`, `GET /api/budget`, `PUT /api/budget/{id}` | `budget_db.budgets` | - | `BudgetDashboardPage.tsx` | COMMISSIONER / ADMIN | Fiscal year, department budget limits | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Department & Scheme Budget Allocations | `budget-service` | `POST /api/budget/allocation`, `GET /api/budget/allocation/budget/{budgetId}` | `budget_db.budget_allocations` | - | `BudgetAllocationDialog.tsx` | COMMISSIONER / ADMIN | Allocated amount $\le$ remaining budget | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Departmental Expense Management | `budget-service` | `POST /api/expenses`, `GET /api/expenses`, `GET /api/expenses/budget/{budgetId}` | `budget_db.expenses` | - | `ExpenseManagementPage.tsx` | OFFICER / ADMIN | Valid expense category & budget ID | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Automated Fund Distribution & Payment Tracking | `budget-service` | `POST /api/fund-distributions`, `PUT /api/fund-distributions/{id}/complete`, `PUT /api/fund-distributions/{id}/fail` | `budget_db.fund_distribution` | - | `FundDistributionPage.tsx` | COMMISSIONER / ADMIN | Beneficiary verification, bank details | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Financial Compliance & Audit Trail | `budget-service` | `GET /api/audit`, `GET /api/audit/entity/{entityType}` | `budget_db.audit_logs` | - | `AuditLogsPage.tsx` | COMMISSIONER / ADMIN | Immutable transaction timestamps | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
| **M3** | Budget Analytics & Utilization Dashboard | `budget-service` | `GET /api/budget/dashboard`, `GET /api/analytics/dashboard` | `budget_db` tables | - | `BudgetDashboardPage.tsx` | COMMISSIONER / ADMIN | Utilization %, remaining balances | `verify_m3_e2e.py` (All steps verified) | **COMPLETE** |
...
| Milestone | Requirement | Backend Service | API Endpoint | Database | Kafka Topic/Event | Frontend Page/Component | Role | Validation | Testing Evidence | Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :---: |
| **M4** | OpenFeign Multi-Service Data Aggregation | `reporting-service` | Internal OpenFeign Clients (`CitizenClient`, `GrievanceClient`, `ServiceManagementClient`, `WelfareClient`, `BudgetClient`) | Aggregates all 5 DBs | - | Reporting Service Pipeline | SYSTEM | Fallbacks for resilient aggregation | `mvn clean compile` BUILD SUCCESS; Eureka registration verified; OpenFeign fallbacks active | **COMPLETE** |
| **M4** | Executive Governance Dashboard | `reporting-service` | `GET /api/reports/dashboard` | Consolidated | - | `ExecutiveDashboardPage.tsx` | COMMISSIONER / ADMIN | Cross-service KPI cards, resolution rates | `verify_m4_e2e.py` ADMIN→200, CITIZEN→403; all DTO keys verified; `npm run build` ✓ | **COMPLETE** |
| **M4** | Citizen Demographics Analytics Report | `reporting-service` | `GET /api/reports/citizens` | `citizen_db` | - | `CitizenReportPage.tsx` | COMMISSIONER / ADMIN | Ward distribution, active/inactive ratios | `verify_m4_e2e.py` ADMIN→200, CITIZEN→403; wardDistribution, activeCitizens validated; `npm run build` ✓ | **COMPLETE** |
| **M4** | Grievance Redressal & SLA Performance Report | `reporting-service` | `GET /api/reports/grievances` | `grievance_db` | - | `GrievanceReportPage.tsx` | COMMISSIONER / ADMIN | Category breakdown, priority SLA | `verify_m4_e2e.py` ADMIN→200, CITIZEN→403; categoryBreakdown, priorityBreakdown validated; `npm run build` ✓ | **COMPLETE** |
| **M4** | Municipal Revenue & Collections Report | `reporting-service` | `GET /api/reports/revenue` | `service_management_db` | - | `RevenueReportPage.tsx` | COMMISSIONER / ADMIN | Department collections, monthly totals | `verify_m4_e2e.py` ADMIN→200, CITIZEN→403; revenueByDepartment, certificatesIssued validated; `npm run build` ✓ | **COMPLETE** |
| **M4** | Budget Allocation & Welfare Disbursement Report | `reporting-service` | `GET /api/reports/budget` | `budget_db`, `welfare_db` | - | `BudgetReportPage.tsx` | COMMISSIONER / ADMIN | Allocation vs utilization, payments | `verify_m4_e2e.py` ADMIN→200, CITIZEN→403; utilizationPercentage, totalDistributedWelfare validated; `npm run build` ✓ | **COMPLETE** |
| **M4** | Cross-Department Performance Scorecard | `reporting-service` | `GET /api/reports/performance` | Cross-domain | - | `DepartmentPerformancePage.tsx` | COMMISSIONER / ADMIN | Department performance scores | `verify_m4_e2e.py` ADMIN→200, CITIZEN→403; departmentScorecards validated; `npm run build` ✓ | **COMPLETE** |
...
| **Reporting Service** | `8087` | `reporting-service` (Spring Boot, Eureka-registered, RBAC secured) | Gateway route `/api/reports/**`→`REPORTING-SERVICE`; Eureka confirmed; ADMIN/COMMISSIONER→200; CITIZEN→403 | **VERIFIED & UP** |
| **Frontend React 19 + TypeScript** | `5173` | `civicpulse-frontend` (Vite) | Production build `✓ 1049 modules transformed` | **VERIFIED & READY** |
